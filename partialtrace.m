function rho = partialtrace(p,sys,dim)
...
%    TM: SYS is a list of which subsystems to trace over.  E.g. if the
%    first subspace is to be traced over, SYS = 1.  If the first and
%    second are to be traced over, SYS = [1,2].
...
if any(size(p) == 1)
  ...
  rho = reshape(permute(reshape(p,rdim),perm),[dimkeep,dimtrace]);
  rho = rho*rho';
  ...
  rho = reshape(permute(reshape(p,[rdim,rdim]),perm),...
              [dimkeep,dimkeep,dimtrace^2]);
  rho = sum(rho(:,:,[1:dimtrace+1:dimtrace^2]),3);
